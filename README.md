...
정책(MVP):
- **게스트**: 기존처럼 브라우저 LocalStorage에만 저장
- **로그인 사용자**: 설정 탭에서 “클라우드 저장/불러오기”를 통해 Supabase에 저장
...
이후 게임 설정 탭(👤 계정)에서:
- **☁️ 클라우드 저장**: 현재 로컬 저장을 클라우드로 업로드
- **☁️ 클라우드 불러오기**: 클라우드 저장을 로컬에 덮어쓰기 후 새로고침
- **클라우드 자동 동기화(토글)**: 켜면 로컬 저장을 일정 주기로 클라우드에 자동 업로드(기본값: OFF, 수동 업로드 성공 시 “켜기” 제안)
