...
export const League: React.FC<LeagueProps> = ({ state, onNavigate }) => {
  ...
  const getFightersByWC = (wc: string) => {
      return Object.values(state.fighters)
        ...
        // Spec says: "리그/Rankings logic is strictly statistical screen"
  ...
  return (
    <div className="fade-in">
        ...
        {activeTab === 'PROMOTIONS' && (
             <Card title={t("Global Promotion Standings")}>
                <table className="data-table" style={{ width: '100%', borderCollapse: 'collapse' }}>
                    ...
                    <tr style={{ borderBottom: '1px solid rgba(255,255,255,0.1)', textAlign: 'left', color: 'var(--text-muted)', fontSize: '0.9rem' }}>
                        ...
                        <th style={{ padding: '12px', textAlign: 'right' }}>{t("Active Budget")}</th>
        ...
        {activeTab === 'FIGHTERS' && (
            <Card title={`${t("Top 10 Rankings")} - ${selectedWC}`}>
                 <div style={{ padding: '16px', borderBottom: '1px solid rgba(255,255,255,0.1)' }}>
                     <label style={{ marginRight: '12px', color: 'var(--text-muted)' }}>{t("Select Weight Class")}:</label>
                     <select 
                        value={selectedWC} 
                 ...
                 <table className="data-table" style={{ width: '100%', borderCollapse: 'collapse' }}>
                    ...
                    <tr style={{ borderBottom: '1px solid rgba(255,255,255,0.1)', textAlign: 'left', color: 'var(--text-muted)', fontSize: '0.9rem' }}>
                        <th style={{ padding: '12px' }}>{t("Rank")}</th>
                        <th style={{ padding: '12px' }}>{t("Fighter")}</th>
                        <th style={{ padding: '12px' }}>{t("Record")}</th>
                        <th style={{ padding: '12px' }}>{t("Promotion")}</th>
                        <th style={{ padding: '12px', textAlign: 'right' }}>{t("Ticket Power")}</th>
                    </tr>
                    </thead>
                    <tbody>
                    {topFighters.map((f, idx) => {
                        ...
                        return (
                            <tr key={f.id} style={{ borderBottom: '1px solid rgba(255,255,255,0.05)' }}>
                                <td style={{ padding: '12px' }}>{idx + 1}</td>
                                <td style={{ padding: '12px', fontWeight: 600 }}>{formatFighterName(f)}</td>
                                <td style={{ padding: '12px' }}>{f.record.w}-{f.record.l}</td>
                                <td style={{ padding: '12px' }}>
                                    <span style={{ color: 'var(--text-muted)' }}>{prom ? prom.name : t("Free Agent")}</span>
