...
import { pickUniqueNickname } from '../utils/nickname';
...
export function bootstrapGame(seed: number = 12345): GameState {
  ...
  // Nickname registry for uniqueness
  ...
  (Object.keys(countByWC) as WeightClass[]).forEach(wc => {
    ...
    for (let i = 0; i < count; i++) {
      ...
      const fighter: Fighter = {
        id: fighterId,
        nickname: pickUniqueNickname(fighterId, gym.country, usedNicks),
        age: age,
  ...
  return {
    ...
    season: {
      eventsPlanned: 12, // P0: Initialize with valid number to prevent NaN
      eventsCompleted: 0,
      events: [],
    },
    market: {
